...
If you have a `public` state variable of array type, then it only returns
individual elements to reduce the gas cost of returning an entire array. You
use parameters to specify which individual element to return, for example
...
By adding the ``public`` keyword to the ``myArray`` variable, the ``getArray``
function behaves in a similar way to the function generated by the compiler and
you are able to access the entire array.
...
Other code executed once right before the constructor always generates code to
represent the initial storage state, initialising any state variables you
...
default constructor, which is equivalent to ``constructor() public {}``. For example:
